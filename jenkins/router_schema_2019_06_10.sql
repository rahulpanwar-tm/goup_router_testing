USE goup_router;
insert into status (id,status,message) VALUES (8,7, 'File Contains blank fileds');

drop procedure if exists goup_router_sim_inventory_bulk_insert;
DELIMITER $$
CREATE PROCEDURE `goup_router_sim_inventory_bulk_insert`(IN `in_user_key` VARCHAR(256), IN `in_user_id` VARCHAR(256), IN `in_file_id` INT(16)
)
...
BEGIN 
	...
	*/
        ...
        if(select count(id) from bulk_upload_sim_inventory where file_id=in_file_id)=0 then
        	...
        	if  (SELECT count(sim_id) FROM bulk_upload_sim_inventory where file_id=in_file_id and 
			  (TRIM(TRAILING '\r\n' FROM bulk_upload_sim_inventory.wso2_appilication_name)='' OR 
		TRIM(TRAILING '\r\n' FROM bulk_upload_sim_inventory.sim_uid)=''
      OR TRIM(TRAILING '\r\n' FROM bulk_upload_sim_inventory.name)='' OR 
		TRIM(TRAILING '\r\n' FROM bulk_upload_sim_inventory.sim_id)='' OR 
		TRIM(TRAILING '\r\n' FROM bulk_upload_sim_inventory.`type`)='' OR 
		bulk_upload_sim_inventory.wso2_appilication_name is NULL OR 
		bulk_upload_sim_inventory.sim_uid is NULL OR 
		bulk_upload_sim_inventory.name is NULL OR 
		bulk_upload_sim_inventory.sim_id is NULL OR 
		bulk_upload_sim_inventory.`type` is NULL
     )) >0
        then 
