USE goup_router;
INSERT INTO `status` (`status`, `message`) VALUES ('6', 'File Contains duplicate data');
...
BEGIN 
	...
	*/
      ...
      if(SELECT count(sim_id) FROM bulk_upload_sim_inventory group by sim_id,type,sim_uid having count(sim_id) >1 limit 1 ) >0
        then 
